...
public class BlueIceMiner extends Module {
    ...
    int echestSlot = -1;
    public static int mineSwitch = 50;
    ...
    private final Setting<Integer> delay = sgMining.add(new IntSetting.Builder()
            .name("blue-ice-break-time")
            .description("Ticks to break one blue ice. Adjust according to lag.")
            .defaultValue(10)
            .min(8)
            .max(20)
            .sliderRange(8, 20)
    ...
    public ArrayList<BlockPos> getBlueIceInRange() {
        ...
        for (int j = 1; j <= 2; j++) {
            for (int x = sx - j; x <= sx + j; x++) {
                for (int z = sz - j; z <= sz + j; z++) {
                    for (int y = sy + j + 1; y >= sy - j; y--) {
                        BlockPos blockPos = new BlockPos(x, y, z);
                        if (mc.world.getBlockState(blockPos).getBlock() == Blocks.BLUE_ICE) {
                            if (y != sy - 1 && PlayerUtils.isWithin(blockPos, j + 0.4) && !blockPosList.contains(blockPos)) {
                                // && Math.abs(sz - z) + Math.abs(sx - z) + Math.abs(sy - y) <= j
        ...
        BlockPos blockPos = new BlockPos(sx, sy - 1, sz);
        if (mc.world.getBlockState(blockPos).getBlock() == Blocks.BLUE_ICE) {
            blockPosList.add(blockPos);
        }
    ...
    private void handleMineBlueIceInRange() {
        ...
        if (range.isEmpty()) {
            ...
            if (mineDelay > delay.get()) {
                mineDelay = 0;
                mineSwitch = (int) Math.random()*50;
                return;
            }
            //a delay every few seconds to simulate turning on and off
            if (tick % (mineSwitch + 70) < 20) {
                return;
            }
            mineDelay ++;
            lookAtBlock(range.get(0));
            BlockUtils.breakBlock(range.get(0), true);
